...
Nim also has a loveable syntax, and it should feel relatively familiar for Python programmers. Additionally, they even provide [Nim for Python Programmers](https://github.com/nim-lang/Nim/wiki/Nim-for-Python-Programmers) so that you can get a good glance at the language. Though, typing procedure parameters and return types is a bit weird. Even with suffix type annotation, they still require `auto` to enable type inference, which is weird. The equivalency `free_func(value)` and `value.free_func()` (call Uniformed Function Call Syntax, UFCS) is also uncommon, although useful in a lot of scenario. Its downside is that it could clutter autocompletion.
